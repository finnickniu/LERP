...
# from text_lab_event.text_event_bert import LEAM
...
### transpose permute 是维度交换, view 可以用于元素压缩和展开
    def forward(self,token_map,text_x,event_token,label_token,task_token,lab_x,length,fixed_label_embedding,fixed_task_embedding,time_stamp,Fixed,Flatten,mode='fusion'):
        text_pred,weights,c,text_label,event_weight,weighted_event,text_event = self.text_encoder(token_map,text_x,event_token,label_token,task_token,time_stamp)
        ...
        f_x = torch.cat((text_pred,weighted_event),-1)

        # ######### early fusion #########
        # f_x = F.leaky_relu(self.drop_out(self.text_encoder.fc(self.drop_out(self.mlp(f_x)))))
        ...
        # f_x = self.drop_out(self.mlp(f_x))
        # f_x =  F.leaky_relu(self.drop_out(self.mlp(f_x)))
        # output = self.sigmoid(self.drop_out(self.text_fc(f_x.squeeze(1))))
        c_o = c
        c = self.text_encoder.dropout(self.text_encoder.fc(c))
        return output,c,text_label,weights,weighted_event,event_weight,text_event
