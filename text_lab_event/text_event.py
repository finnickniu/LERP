...
###########################################################
    ...
    def forward(self, token_map,text,event_token,c0,task_token,time_stamp):
        ...
        b,u,g_l = self.scaled_attention(v,c)
        e_a,u_e,g_e =  self.scaled_attention(v,e,flag='event')
        ...
        ######
        _,_,g_e = self.scaled_attention(e,c)
        return b,v,c,g_l,e_a,g_e
...
class LEAM(nn.Module):
    ...
    def forward(self, token_map,text,event_token,label_token,task_token,time_stamp):
        weight, embed,c,text_label,event_weight,text_event = self.compat_model(token_map,text,event_token,label_token,task_token,time_stamp)
        weighted_embed_label = self.dropout(weight*embed) 
        weighted_embed_sumed_label = weighted_embed_label.sum(1)

        ################# not early fusion #######################
        # weighted_embed_sumed_label = self.dropout(self.fc(weighted_embed_sumed_label)).unsqueeze(1)
        ...
        ### three modality
        return weighted_embed_sumed_label,weight,c,text_label,event_weight,weighted_embed_sumed_event,text_event
